...
class Admin_model extends CI_Model{
	...
	public function get_affiliated_institutes_info($status ='',$inst_id = "")
	{
		if (empty($status)) {
			$this->db->select('afinfo.*',false);
			$this->db->from('affiliation_info AS afinfo');
			if (!empty($inst_id)) {
				$this->db->where('inst_reg_no',$inst_id);
			}
			$query = $this->db->get();
		}else{
			$this->db->select('afinfo.*',false);
	...
	public function get_affiliated_institutes_info_old($status ='')
	{
		if (empty($status)) {
		...
		if ($query->num_rows() > 0) {
			...
			{					
				if ($rec['attempt_no'] == "first") {
					...
					$returnable_response[] = $sb_query->result_array()[0]; 
